...
export function PhotoCarouselManager({ photoCarousel }: PhotoCarouselManagerProps) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (newPhoto) => {
      ...
      queryClient.setQueryData(["/api/admin/photo-carousel"], (old: any[] = []) => {
        return [...old, newPhoto[0]];
      });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<PhotoForm> }) => {
      ...
      return { id, updatedPhoto };
    },
    onSuccess: ({ id, updatedPhoto }) => {
      // Atualiza cache manualmente EVITANDO qualquer invalidação
      queryClient.setQueryData(["/api/admin/photo-carousel"], (old: any[] = []) => {
        return old.map(photo => photo.id === id ? updatedPhoto[0] : photo);
      });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      // Atualiza cache manualmente EVITANDO qualquer invalidação
      queryClient.setQueryData(["/api/admin/photo-carousel"], (old: any[] = []) => {
        return old.filter(photo => photo.id !== deletedId);
      });
