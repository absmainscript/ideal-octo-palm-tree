...
export function ServicesManager({ services }: ServicesManagerProps) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (newService) => {
      // Atualiza cache manualmente sem invalidação
      queryClient.setQueryData(["/api/admin/services"], (old: Service[] = []) => {
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over?.id && over) {
      ...
      queryClient.setQueryData(["/api/admin/services"], reorderedServices);
      ...
      Promise.all(updatePromises).then(() => {
        ...
        queryClient.setQueryData(["/api/admin/services"], services);
      });
    }
  };
