...
export function TestimonialsManager({ testimonials = [] }: { testimonials: Testimonial[] }) {
  ...
  // Atualiza dados locais quando props mudam
  React.useEffect(() => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (newTestimonial) => {
      ...
      // Atualiza cache manualmente EVITANDO qualquer invalidação
      ...
      // Fecha o diálogo imediatamente sem delay
  ...
  const reorderMutation = useMutation({
    ...
    onSuccess: (reorderedData) => {
      // Dados locais já estão atualizados pelo drag & drop
      // Só precisa atualizar o cache
  ...
  const handleDialogOpenChange = (open: boolean) => {
    // Só permite fechar se não estiver processando uma mutation
    if (!open && isLoading) {
      return;
    }
    ...
    if (!open) {
      // Reset imediatamente quando fecha manualmente
      form.reset({
        name: "",
        service: "",
        testimonial: "",
